...
@pytest.mark.a11y
async def test_add_employee_default(page, base_url, credentials):
    await login_as_admin(page, base_url, credentials)
    await page.goto(f"{base_url}add-employee.php", wait_until="domcontentloaded")
    await page.wait_for_timeout(500)
    await run_axe_scan(page, "Add Employee — Default State")
...
@pytest.mark.a11y
async def test_add_employee_validation_errors(page, base_url, credentials):
    await login_as_admin(page, base_url, credentials)
    await page.goto(f"{base_url}add-employee.php", wait_until="domcontentloaded")
    await page.get_by_role("button", name="Add Employee").click()
    await page.wait_for_timeout(500)
    await run_axe_scan(page, "Add Employee — Validation Error State")